    ...
    <div className="min-h-screen overflow-x-hidden bg-background text-foreground" data-testid="portfolio-app">
      ...
      <header className="sticky top-4 z-40 mx-4 rounded-full border-2 border-ink bg-card/90 shadow-[4px_4px_0_var(--ink)] backdrop-blur-xl sm:mx-8 lg:mx-auto lg:max-w-[calc(100%-6rem)]" data-testid="portfolio-header">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-5 py-3 sm:px-6" data-testid="portfolio-navigation">
          <a href="#top" className="flex items-center gap-2 text-sm font-semibold tracking-tight text-ink" data-testid="nav-brand-link">
            <span className="grid size-8 place-items-center rounded-full border-2 border-ink bg-tropical-rain font-heading text-xs font-bold text-[#25151f]" data-testid="nav-brand-mark">HS</span>
      ...
      <main id="top" data-testid="portfolio-main">
        <section className="relative mx-auto grid max-w-7xl gap-12 px-5 pb-20 pt-20 sm:px-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-end lg:gap-20 lg:px-12 lg:pb-28 lg:pt-28" data-testid="hero-section">
          <div className="absolute -left-28 top-24 -z-0 size-72 rounded-full bg-pure-sun/75" aria-hidden="true" />
          <div className="absolute -right-36 top-10 -z-0 size-64 rounded-full bg-autumn-lavender/90" aria-hidden="true" />
          <div className="absolute left-[45%] top-14 -z-0 size-24 rotate-12 rounded-[2rem] bg-limeade/80" aria-hidden="true" />
          <div className="absolute left-[38%] top-[58%] -z-0 size-20 rounded-full bg-bubble-gum/70" aria-hidden="true" />
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.12 }} className="relative z-10 lg:pb-8" data-testid="hero-portrait-panel">
            <div className="relative ml-auto max-w-md" data-testid="hero-portrait-frame">
              <div className="absolute -right-8 -top-8 size-28 rounded-full border-2 border-ink bg-pure-sun" aria-hidden="true" />
              <div className="absolute -bottom-10 right-8 size-20 rounded-full border-2 border-ink bg-tropical-rain" aria-hidden="true" />
              <div className="aspect-[0.82/1] rotate-2 overflow-hidden rounded-[3.25rem] border-[3px] border-ink bg-peony-bundle shadow-[16px_16px_0_var(--bubble-gum)]" data-testid="hero-portrait-image-wrap"><img src={heroImage} alt="Hiba Salim, MBA candidate and product manager" className="h-full w-full object-cover object-top" data-testid="hero-portrait-image" /></div>
              <div className="sticker-float absolute -bottom-8 -left-8 w-52 rounded-[2rem] border-2 border-ink bg-airplane-view p-5 shadow-[5px_5px_0_var(--pure-sun)]" data-testid="hero-note-card"><Sparkles className="size-5 text-tomato-jam" aria-hidden="true" /><p className="mt-3 font-heading text-lg leading-tight text-[#25151f]" data-testid="hero-note-text">Sketchbook → spreadsheet → strategy deck.</p><p className="mt-2 text-xs leading-5 text-[#25151f]/70" data-testid="hero-note-supporting-text">Plot twist: it all connects.</p></div>
            </div>
          </motion.div>
        </section>
        ...
        <section id="projects" className="border-y-2 border-ink bg-airplane-view text-[#25151f]" data-testid="projects-section">
          <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:px-12 lg:py-28">
            <div className="flex flex-col gap-10 lg:flex-row lg:items-end lg:justify-between" data-testid="projects-header"><div><p className="font-mono text-[10px] uppercase tracking-[0.22em] text-tomato-jam" data-testid="projects-eyebrow">03 / The work — with receipts</p><h2 className="mt-5 max-w-2xl font-heading text-4xl font-semibold tracking-tight sm:text-6xl" data-testid="projects-heading">Big questions. Real numbers. <span className="italic text-bubble-gum" data-testid="projects-heading-accent">No vague strategy fog.</span></h2><p className="mt-5 max-w-xl text-base leading-7 text-[#25151f]/70" data-testid="projects-description">Open a card for the model, the messy bit, the recommendation, and what the numbers actually changed.</p></div><div className="flex flex-wrap gap-2" data-testid="project-filter-list"><button type="button" onClick={() => setActiveCategory("All")} className={`rounded-full border-2 border-ink px-4 py-2 text-xs font-semibold transition-colors duration-200 ${activeCategory === "All" ? "bg-pure-sun text-[#25151f]" : "bg-white/55 text-[#25151f] hover:bg-peony-bundle"}`} data-testid="project-filter-all">All the good stuff <span data-testid="project-count-all">{projects.length}</span></button>{categories.map((category) => <button type="button" key={category} onClick={() => setActiveCategory(category)} className={`rounded-full border-2 border-ink px-4 py-2 text-xs font-semibold transition-colors duration-200 ${activeCategory === category ? "bg-pure-sun text-[#25151f]" : "bg-white/55 text-[#25151f] hover:bg-peony-bundle"}`} data-testid={`project-filter-${category.toLowerCase()}`}>{category}</button>)}</div></div>
            ...
            <div className="mt-12 flex items-center justify-between border-t-2 border-ink/20 pt-6" data-testid="projects-footer"><p className="text-sm text-[#25151f]/60" data-testid="projects-footer-note">{projects.length} projects. Several spreadsheets. Zero mystery metrics.</p><a href="#contact" className="inline-flex items-center gap-2 text-sm font-semibold text-[#25151f] hover:text-tomato-jam" data-testid="projects-contact-link">Got a knot to untangle? <ArrowUpRight className="size-4" aria-hidden="true" /></a></div>
          </div>
        </section>
