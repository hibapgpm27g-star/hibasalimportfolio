...
const experience = [
  ...
  { id: "footwear", period: "2021 — 2024", role: "Footwear Designer & Product Manager", place: "Retail & Apparel · Footwear Division", detail: "Used design thinking and stakeholder management to own the lifecycle from concept sketch through sourcing, vendor negotiation, and retail launch." },
  { id: "bdes", period: "2017 — 2021", role: "Bachelor of Design", place: "FDDI Chennai", detail: "Footwear design, production technology, ergonomics, and prototype testing." },
];
      ...
      <main id="top" data-testid="portfolio-main">
        <section className="relative mx-auto grid max-w-7xl gap-12 px-5 pb-20 pt-20 sm:px-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-end lg:gap-20 lg:px-12 lg:pb-28 lg:pt-28" data-testid="hero-section">
          ...
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="relative z-10" data-testid="hero-copy">
            ...
            <p className="mt-8 max-w-xl text-base leading-7 text-muted-foreground sm:text-lg" data-testid="hero-description">Hi, I'm Hiba — an MBA candidate who started with footwear collections and somehow ended up happily inside regression models, retail margins, and last-mile routes. The through-line? I like turning complexity into a plan people can actually use.</p>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.12 }} className="relative z-10 lg:pb-8" data-testid="hero-portrait-panel">
            <div className="relative ml-auto max-w-md" data-testid="hero-portrait-frame">
              ...
              <div className="aspect-[0.82/1] rotate-2 overflow-hidden rounded-[2.25rem] border-2 border-ink bg-muted shadow-[14px_14px_0_var(--pop-pink)]" data-testid="hero-portrait-image-wrap"><img src={heroImage} alt="Hiba Salim, MBA candidate and product manager" className="h-full w-full object-cover grayscale-[0.08]" data-testid="hero-portrait-image" /></div>
        ...
        <section id="about" className="border-y border-border bg-surface-subtle" data-testid="about-section">
          <div className="mx-auto grid max-w-7xl gap-12 px-5 py-20 sm:px-8 lg:grid-cols-[0.75fr_1.25fr] lg:gap-24 lg:px-12 lg:py-28">
            ...
            <div data-testid="about-content"><p className="max-w-3xl text-xl leading-9 text-ink sm:text-2xl" data-testid="about-summary">Footwear taught me that a good idea means very little until materials, vendors, timelines, people, and customers all agree to cooperate. (They rarely do on the first try.)</p><p className="mt-7 max-w-2xl text-base leading-7 text-muted-foreground" data-testid="about-detail">Nearly three years of owning products from concept to retail gave me an operator's view of business. My MBA is adding the strategy, analytics, and consulting language to explain what I already loved doing: finding the knot, untangling it, and getting everyone moving in the same direction.</p><div className="mt-10 flex flex-wrap gap-3" data-testid="about-tags">{["Gurgaon, India", "Product brain", "Customer curious", "Spreadsheet friendly"].map((tag) => <span key={tag} className="-rotate-1 rounded-full border-2 border-ink bg-card px-4 py-2 text-xs font-semibold text-ink shadow-[2px_2px_0_var(--pop-yellow)]" data-testid={`about-tag-${tag.toLowerCase().replaceAll(", ", "-").replaceAll(" ", "-")}`}>{tag}</span>)}</div></div>
          </div>
        </section>

        <section className="border-b-2 border-ink bg-pop-lime text-[#25151f]" data-testid="principles-strip">
        ...
        <section id="projects" className="bg-ink text-background" data-testid="projects-section">
          <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:px-12 lg:py-28">
            <div className="flex flex-col gap-10 lg:flex-row lg:items-end lg:justify-between" data-testid="projects-header"><div><p className="eyebrow text-pop-yellow" data-testid="projects-eyebrow">03 / The work — with receipts</p><h2 className="mt-5 max-w-2xl font-heading text-4xl font-semibold tracking-tight text-background sm:text-6xl" data-testid="projects-heading">Big questions. Real numbers. <span className="italic text-pop-pink" data-testid="projects-heading-accent">No vague strategy fog.</span></h2><p className="mt-5 max-w-xl text-base leading-7 text-background/65" data-testid="projects-description">Open a card for the model, the messy bit, the recommendation, and what the numbers actually changed.</p></div><div className="flex flex-wrap gap-2" data-testid="project-filter-list"><button type="button" onClick={() => setActiveCategory("All")} className={`rounded-full border border-white/20 px-4 py-2 text-xs font-semibold transition-colors duration-200 ${activeCategory === "All" ? "bg-pop-lime text-[#25151f]" : "bg-white/10 text-background/70 hover:bg-white/15"}`} data-testid="project-filter-all">All the good stuff <span data-testid="project-count-all">{projects.length}</span></button>{categories.map((category) => <button type="button" key={category} onClick={() => setActiveCategory(category)} className={`rounded-full border border-white/20 px-4 py-2 text-xs font-semibold transition-colors duration-200 ${activeCategory === category ? "bg-pop-lime text-[#25151f]" : "bg-white/10 text-background/70 hover:bg-white/15"}`} data-testid={`project-filter-${category.toLowerCase()}`}>{category}</button>)}</div></div>
        ...
        <section id="experience" className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:px-12 lg:py-28" data-testid="experience-section"><div className="grid gap-12 lg:grid-cols-[0.7fr_1.3fr]"><div><p className="eyebrow" data-testid="experience-eyebrow">04 / How I got here</p><h2 className="mt-5 max-w-sm font-heading text-3xl font-semibold tracking-tight text-ink sm:text-5xl" data-testid="experience-heading">A career path with range. <span className="italic text-brand">And a point.</span></h2></div><div className="divide-y-2 divide-ink/15 border-y-2 border-ink/15" data-testid="experience-timeline">{experience.map((entry) => <article key={entry.id} className="grid gap-4 py-7 sm:grid-cols-[0.45fr_1fr] sm:gap-10" data-testid={`experience-entry-${entry.id}`}><p className="font-mono text-xs uppercase tracking-[0.15em] text-brand" data-testid={`experience-period-${entry.id}`}>{entry.period}</p><div><h3 className="font-heading text-xl font-semibold text-ink" data-testid={`experience-role-${entry.id}`}>{entry.role}</h3><p className="mt-1 text-sm font-medium text-muted-foreground" data-testid={`experience-place-${entry.id}`}>{entry.place}</p><p className="mt-4 max-w-xl text-sm leading-6 text-muted-foreground" data-testid={`experience-detail-${entry.id}`}>{entry.detail}</p></div></article>)}</div></div></section>

        <section id="skills" className="border-y-2 border-ink bg-pop-pink text-[#25151f]" data-testid="skills-section"><div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:px-12 lg:py-24"><div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-end"><div><p className="font-mono text-[10px] uppercase tracking-[0.22em]" data-testid="skills-eyebrow">05 / Tabs open in my brain</p><h2 className="mt-5 font-heading text-3xl font-semibold tracking-tight sm:text-5xl" data-testid="skills-heading">Strategy in one hand. Spreadsheet in the other.</h2></div><span className="font-mono text-xs uppercase tracking-[0.18em] opacity-60" data-testid="skills-caption">Always learning / immediately applying</span></div><div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4" data-testid="skills-grid">{skills.map((skill, index) => <div key={skill.group} className="rounded-2xl border-2 border-[#25151f] bg-[#fff8ed] p-6 shadow-[5px_5px_0_#25151f]" data-testid={`skill-card-${index + 1}`}><span className="font-mono text-xs text-brand" data-testid={`skill-index-${index + 1}`}>0{index + 1}</span><h3 className="mt-10 font-heading text-2xl font-semibold" data-testid={`skill-group-${index + 1}`}>{skill.group}</h3><p className="mt-4 text-sm leading-6 opacity-70" data-testid={`skill-items-${index + 1}`}>{skill.items}</p></div>)}</div></div></section>
        ...
        <section id="contact" className="bg-ink text-background" data-testid="contact-section"><div className="mx-auto grid max-w-7xl gap-14 px-5 py-20 sm:px-8 lg:grid-cols-[0.9fr_1.1fr] lg:gap-24 lg:px-12 lg:py-28"><div><p className="font-mono text-[10px] uppercase tracking-[0.22em] text-pop-yellow" data-testid="contact-eyebrow">07 / Your move</p><h2 className="mt-5 max-w-lg font-heading text-4xl font-semibold tracking-tight sm:text-6xl" data-testid="contact-heading">Got a problem with too many moving parts? <span className="italic text-pop-pink">My kind of conversation.</span></h2><p className="mt-6 max-w-sm text-base leading-7 text-background/60" data-testid="contact-description">Recruiting for an MBA role, building something ambitious, or simply comparing notes? Say hello. Bonus points for a genuinely messy brief.</p><div className="mt-10 space-y-4" data-testid="contact-links"><a href="mailto:hiba.salim@email.com" className="flex items-center gap-3 text-sm text-background/75 hover:text-pop-yellow" data-testid="contact-email-link"><Mail className="size-4 text-pop-yellow" aria-hidden="true" /> hiba.salim@email.com</a><a href="https://www.linkedin.com/" target="_blank" rel="noreferrer" className="flex items-center gap-3 text-sm text-background/75 hover:text-pop-yellow" data-testid="contact-linkedin-link"><ExternalLink className="size-4 text-pop-yellow" aria-hidden="true" /> linkedin.com/in/hibasalim</a><span className="flex items-center gap-3 text-sm text-background/75" data-testid="contact-location"><MapPin className="size-4 text-pop-yellow" aria-hidden="true" /> Gurgaon, India</span></div></div><form onSubmit={(event) => { event.preventDefault(); toast.success("Message ready", { description: "Thanks — Hiba will get back to you at the email you shared." }); }} className="rotate-[0.5deg] rounded-3xl border-2 border-background/60 bg-background p-6 text-[#25151f] shadow-[8px_8px_0_var(--pop-pink)] sm:p-8" data-testid="contact-form"><div className="grid gap-5 sm:grid-cols-2"><label className="grid gap-2 text-xs font-semibold uppercase tracking-[0.12em] opacity-60" data-testid="contact-name-field"><span data-testid="contact-name-label">Your name</span><input required name="name" className="h-11 rounded-xl border-2 border-[#25151f]/20 bg-white/60 px-3 text-sm outline-none placeholder:text-[#25151f]/35 focus:border-brand focus:ring-3 focus:ring-brand/20" placeholder="The person behind the email" data-testid="contact-name-input" /></label><label className="grid gap-2 text-xs font-semibold uppercase tracking-[0.12em] opacity-60" data-testid="contact-email-field"><span data-testid="contact-email-label">Best email</span><input required type="email" name="email" className="h-11 rounded-xl border-2 border-[#25151f]/20 bg-white/60 px-3 text-sm outline-none placeholder:text-[#25151f]/35 focus:border-brand focus:ring-3 focus:ring-brand/20" placeholder="you@company.com" data-testid="contact-email-input" /></label></div><label className="mt-5 grid gap-2 text-xs font-semibold uppercase tracking-[0.12em] opacity-60" data-testid="contact-message-field"><span data-testid="contact-message-label">What are we untangling?</span><textarea required name="message" className="min-h-32 resize-y rounded-xl border-2 border-[#25151f]/20 bg-white/60 px-3 py-3 text-sm outline-none placeholder:text-[#25151f]/35 focus:border-brand focus:ring-3 focus:ring-brand/20" placeholder="Give me the interesting version..." data-testid="contact-message-input" /></label><Button type="submit" className="mt-6 w-full border-2 border-[#25151f] bg-pop-lime text-[#25151f] shadow-[4px_4px_0_#25151f] hover:bg-pop-yellow" data-testid="contact-submit-button">Let's make it clearer <ArrowUpRight className="size-4" aria-hidden="true" /></Button></form></div><footer className="mx-auto flex max-w-7xl flex-col gap-3 border-t border-white/10 px-5 py-6 text-xs text-background/40 sm:flex-row sm:items-center sm:justify-between sm:px-8 lg:px-12" data-testid="portfolio-footer"><span data-testid="footer-copyright">© 2026 Hiba Salim · Made with curiosity and an unreasonable number of tabs</span><a href="#top" className="inline-flex items-center gap-2 hover:text-pop-yellow" data-testid="footer-back-to-top-link">Do the scroll again <ChevronRight className="size-3 -rotate-90" aria-hidden="true" /></a></footer></section>
      </main>
